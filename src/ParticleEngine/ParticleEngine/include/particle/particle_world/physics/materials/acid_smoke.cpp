...
void ParticleWorld::calculate_acidSmoke(int row, int col, Ruleset ruleset)
{
	...
	if (ruleset.canUp)
	{
			...
			getParticle(row, col).intValue > 1)
		{
			...
			if (getParticle(row - 1, col).brightnessMultiplier > 0.4f)
			{
		...
		else
		{
			if (
				eatChance == 0 &&
				getParticle(row - 1, col).material != ParticleWorld::Material::Air &&
				getParticle(row - 1, col).material != ParticleWorld::Material::Acid &&
	...
	if (ruleset.canLeft && randomJitter == 0)
	{
			...
			getParticle(row, col).material == self.material)
		{
			...
			eatChance == 0 &&
			getParticle(row, col - 1).material != ParticleWorld::Material::Air &&
			getParticle(row, col - 1).material != ParticleWorld::Material::Acid &&
	...
	particles[get1DIndex(row, col, getColSize())].intValue++;
	if (getParticle(row, col).intValue > 600)
	{
		// eventually kill the particle if it gets to a certain age
		resetParticle(row, col);
	}

	if (getParticle(row, col).intValue % 240 == 0)
	{
